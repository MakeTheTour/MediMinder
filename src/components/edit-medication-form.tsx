...
export function EditMedicationForm({ medicationId }: EditMedicationFormProps) {
  ...
  useEffect(() => {
    ...
    fetchMedicationData();
  }, [user, isGuest, medicationId, form, router, toast, localMedications]);
  ...
  async function onSubmit(values: z.infer<typeof medicationSchema>) {
    if (isGuest) {
        ...
        toast({
            ...
            description: `${values.name} has been updated in your local schedule.`,
        });
        router.push('/medicine');
        return;
    }
    ...
    if (!user) {
        toast({ title: "Not Signed In", description: "Please sign in to update medications.", variant: "destructive" });
        return;
    }
    ...
    try {
        ...
        toast({ title: "Error", description: "Could not update medication. Please try again.", variant: "destructive"});
    }
  }
